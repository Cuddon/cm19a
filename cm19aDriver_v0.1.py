...
"""
A Python interface to CM19a X10 RF Transceiver (USB)
...
Uses: pyUSB, (based on libusb 0.1 series)
For instructions on how to instal pyUSB see: 
...
class CM19aDevice(threading.Thread):
			...
			if not self.paused:
				...
				if data:
					...
					if result:
						...
						#add2log("Command", "Command %s received via cm19a" % result, echo = True)
	...
	def loadProtocol(self):
		...
		if not os.path.isfile(fname):
			add2log("**ERROR**", "Protocol file missing %s" % fname,  echo=True)
			return None
		f = open(fname, "r")
	...
	def encode(self, house_code,  device_number,  on_off):
		"""
			Looks up the PX10 rotocol for the appropriate byte command sequence
		"""
...
def configure():
	...
	cm19a = CM19aDevice(refresh=1)		# Initialise device with 1s refresh rate
...
#Main
# Run this module standone to do these simple tests
if __name__ == '__main__':
	...
	if cm19a.initialised:
		...
		print "Press one or more keys on the x10 remote within the next 30 seconds"
		time.sleep(30)
		print "Receive Queue:",  cm19a.getReceiveQueue()

		print "Press one or more keys on the x10 remote within the next 30 seconds"
		time.sleep(30)
		print "Receive Queue (since the last time we requested the queue):",  cm19a.getReceiveQueue()
